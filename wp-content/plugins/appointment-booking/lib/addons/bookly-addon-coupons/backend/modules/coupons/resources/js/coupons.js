jQuery(function($) {
    ...
    var $serviceFilter    = $('#bookly-filter-service'),
        ...
        $couponServices   = $('#bookly-coupon-services'),
        $couponProviders  = $('#bookly-coupon-providers'),
    ...
    $couponModal.on('show.bs.modal', function () {
        ...
        $couponServices.find('input:checkbox:gt(0)').each(function () {
            this.checked = !row || $.inArray(this.value, data.service_ids) > -1;
        });
        $couponProviders.find('input:checkbox:gt(0)').each(function () {
            this.checked = !row || $.inArray(this.value, data.staff_ids) > -1;
        });
        updateEntitySelector($couponServices, BooklyCouponL10n.services);
        updateEntitySelector($couponProviders, BooklyCouponL10n.staff);
     ...
     * On select service.
     */
    $couponServices.on('change', 'input:checkbox', function() {
        updateEntitySelector($couponServices, BooklyCouponL10n.services, $(this));
    });

    /**
     * On select staff.
     */
    $couponProviders.on('change', 'input:checkbox', function() {
        updateEntitySelector($couponProviders, BooklyCouponL10n.staff, $(this));
    });
