...
class PayuLatam
{
    ...
    public static function replaceData( $form_id )
    {
        ...
        if ( $userData->load() ) {
            ...
            $replacement = array(
                ...
                '%signature%'     => md5( implode( '~', array( $api_key, $merchant_id, $reference_code, $cart_info->getPaymentSystemPayNow(), get_option( 'bookly_pmt_currency' ) ) ) ),
    ...
    public static function renderForm( $form_id, $page_url )
    {
        ...
        if ( ! empty( $replacement ) ) {
            ...
            if ( get_option( 'bookly_payu_latam_send_tax' ) ) {
