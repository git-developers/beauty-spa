...
<div class="tab-pane" id="bookly_settings_cart">
    <form method="post" action="<?php echo esc_url( add_query_arg( 'tab', 'cart' ) ) ?>">
        <?php Selects::renderSingle( 'bookly_cart_enabled', __( 'Cart', 'bookly-cart' ), __( 'If cart is enabled then your clients will be able to book several appointments at once. Please note that WooCommerce integration must be disabled.', 'bookly-cart' ) ) ?>
        <?php Proxy\Shared::renderCartSettings() ?>
