    ...
    public static function aimPayment()
    {
        ...
        if ( $userData->load() ) {
            ...
            if ( $failed_cart_key === null ) {
                ...
                $cart_info->setPaymentMethodSettings( get_option( 'bookly_authorize_net_send_tax' ), 'tax_in_the_price' );
                ...
                $authorize->setField( 'amount', $cart_info->getPaymentSystemPayNow() )
                    ...
                    ->setField( 'tax',          $cart_info->getPaymentSystemPayTax() );
                if ( $last_name ) {
                ...
                if ( $aim_response->approved ) {
                    ...
                    $response = array( 'success' => false, 'error' => Errors::PAYMENT_ERROR, 'error_message' => $aim_response->response_reason_text );
                }
            } else {
                $response = array(
                    'success'         => false,
