        ...
        if ( is_admin() ) {
            add_action( 'admin_notices', function () {
                ...
                if ( $bookly_page ) {
                    ...
                    // Migrator notice.
                    Components\Notices\Migrator::render();
...
Bookly SMS Team.', 'bookly' );
            ...
            if ( get_option('bookly_migration_admin_reminder_after' )
                && get_option( 'bookly_migration_admin_reminder_after' ) < time()
            ) {
                ...
                if ( ! empty ( $admin_emails ) && ! Config::proActive() ) {
                    $headers = array();
...
Recently Bookly was updated to the latest version – 15.2. This version contains <b>Migrator</b> in Bookly menu, which will help you to migrate to the new Bookly – <b>Bookly Pro</b>.

After the migration, all your current settings and appointments will stay unaffected, and you\'ll be able to use them as before.

Please follow a short instruction displayed on <a href="%s">Migrator page</a> to perform a successful migration from Bookly to Bookly Pro. The migration process will be automated and comfortable, and takes just a few minutes.

In case you face any issues or require assistance, do not hesitate to contact Bookly Support team at support@bookly.info.

For more information check this <a href="%s">blog post</a>.

Thank you.', 'bookly' );

                    $message = sprintf( $message,
                        is_multisite() && is_main_site()
                            ? esc_url( network_admin_url( 'admin.php?page=bookly-migrator' ) )
                            : Utils\Common::escAdminUrl( 'bookly-migrator' ),
                        'https://www.booking-wp-plugin.com/bookly-major-update/?utm_campaign=migration&utm_medium=cpc&utm_source=newsletter'
                    );

                    $message = get_option( 'bookly_email_send_as' ) === 'html' ? wpautop( $message ) : $message;

                    wp_mail( $admin_emails, __( 'Major update. Migration from Bookly to Bookly Pro.', 'bookly' ), $message, $headers );
                }
