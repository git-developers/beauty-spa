 ...
 */
class BooklyGoogle_Logger_File extends BooklyGoogle_Logger_Abstract
{
  /**
  ...
  public function __construct(BooklyGoogle_Client $client)
  {
    ...
    $file = $client->getClassConfig('BooklyGoogle_Logger_File', 'file');
    ...
    $mode = $client->getClassConfig('BooklyGoogle_Logger_File', 'mode');
    if (!$mode) {
      $this->mode = $mode;
    }

    $this->lock = (bool) $client->getClassConfig('BooklyGoogle_Logger_File', 'lock');
    $this->file = $file;
  }
  ...
  protected function write($message)
  {
    if (is_string($this->file)) {
      ...
      throw new BooklyGoogle_Logger_Exception('File pointer is no longer available');
    }
