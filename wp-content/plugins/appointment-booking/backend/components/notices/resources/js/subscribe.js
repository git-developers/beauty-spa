jQuery(function ($) {
    ...
    $('#bookly-subscribe-btn').on('click', function () {
        ...
        ladda.start();
        $.post(ajaxurl, {action: 'bookly_subscribe', csrf_token : BooklySubscribeL10n.csrf_token, email: $('#bookly-subscribe-email').val()}, function (response) {
            ladda.stop();
            if (response.success) {
                $alert.alert('close');
    ...
    $alert.on('close.bs.alert', function () {
        $.post(ajaxurl, {action: 'bookly_dismiss_subscribe_notice', csrf_token : BooklySubscribeL10n.csrf_token}, function () {
