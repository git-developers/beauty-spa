jQuery(function ($) {
    ...
    $stars.barrating({
        ...
        onSelect: function (value, text, event) {
            if (value <= 7) {
                ...
                $.post(ajaxurl, {action: 'bookly_nps_send', csrf_token : BooklyNpsL10n.csrf_token, rate: value});
                $quiz.hide();
                $form.hide();
                $thanks.show();
            }
    ...
    $alert.on('close.bs.alert', function () {
        $.post(ajaxurl, {action: 'bookly_dismiss_nps_notice', csrf_token : BooklyNpsL10n.csrf_token}, function () {
